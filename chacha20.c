...
// Función para pasar de entero a littleEndian y desplazar en las posiciones
static void chacha20Serialize(uint32_t in[16], uint8_t output[64]) {
  int i;
  for (i = 0; i < 16; i++) {
...
static void chacha20Block(uint32_t in[16], uint8_t out[64], int rounds) {
  ...
  // Como veía que tras la encriptación, al desencriptar el texto no se asignaba bien los valores, investigué y usé la función memcpy que hace:
  //Copia los valores de num bytes de la ubicación apuntada por la fuente directamente al bloque de memoria apuntado por el destino. Lo que devuelve es el destino
  ...
  // Se llama a la función chacha20Serialize
